...
var chechToken = require('../routes/checkToken')
// connectDB(mongoose)
/* GET home page. */
    ...
    form.parse(req, function (err, fields, files) {
      ...
      keys.forEach(function (key) {
        ...
        if (('.jpg.jpeg.png.gif').indexOf(fileExt.toLowerCase()) === -1) {
          ...
          console.log(fields)
          user.findOneAndUpdate({ '_id': fields._id }, { $set: { 'avatar': filesUrl[0], 'name': fields.name, 'motto': fields.motto, 'gender': parseInt(fields.gender) } }, function (err, uu) {
            if (err) console.log(err)
...
router.loginForAP = function (req, res) {
  ...
  user.findOne({ account: req.body.account, password: req.body.password }, function (err, doc) {
    if (err) console.log(err)
    if (doc == null) {
      res.json({ type: 0, msg: '未找到该用户' })
    } else {
      res.json({ type: 1, msg: '登录成功', user: doc })
    }
  })
}
...
router.getMyTrend = function (req, res) {
  ...
  user.findOne({ 'token': token }, function (err, u) {
    if (err) console.log(err)
    if (u==null) {
      res.send({ 'auth': false, data: u })
    } else {
      trend.find({ 'token': token }, function (err, docs) {
        if (err) console.log(err)
        res.json({ 'auth': true, 'data': docs })
